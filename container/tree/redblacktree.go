...
import (
	...
	"github.com/gocarp/helpers/rwmutex"
...
// GetVar returns a vars.Var with the value by given `key`.
// The returned vars.Var is un-concurrent safe.
func (tree *RedBlackTree) GetVar(key interface{}) *vars.Var {
	return vars.New(tree.Get(key))
}

// GetVarOrSet returns a vars.Var with result from GetVarOrSet.
// The returned vars.Var is un-concurrent safe.
func (tree *RedBlackTree) GetVarOrSet(key interface{}, value interface{}) *vars.Var {
	return vars.New(tree.GetOrSet(key, value))
}

// GetVarOrSetFunc returns a vars.Var with result from GetOrSetFunc.
// The returned vars.Var is un-concurrent safe.
func (tree *RedBlackTree) GetVarOrSetFunc(key interface{}, f func() interface{}) *vars.Var {
	return vars.New(tree.GetOrSetFunc(key, f))
}

// GetVarOrSetFuncLock returns a vars.Var with result from GetOrSetFuncLock.
// The returned vars.Var is un-concurrent safe.
func (tree *RedBlackTree) GetVarOrSetFuncLock(key interface{}, f func() interface{}) *vars.Var {
	return vars.New(tree.GetOrSetFuncLock(key, f))
}
...
func (tree *RedBlackTree) UnmarshalJSON(b []byte) error {
	...
	if tree.comparator == nil {
		tree.comparator = utils.ComparatorString
	}
