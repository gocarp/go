...
import (
	...
	"github.com/gocarp/redis"
	"github.com/gocarp/utils/conv"
)

// AdapterRedis is the cache adapter implements using Redis server.
type AdapterRedis struct {
	redis *redis.Redis
}

// NewAdapterRedis creates and returns a new memory cache object.
func NewAdapterRedis(redis *redis.Redis) Adapter {
	return &AdapterRedis{
		redis: redis,
	}
}
...
func (c *AdapterRedis) Set(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (err error) {
	...
	if value == nil || duration < 0 {
		...
		if duration == 0 {
			...
			_, err = c.redis.Set(ctx, redisKey, value, redis.SetOption{TTLOption: redis.TTLOption{PX: conv.PtrInt64(duration.Milliseconds())}})
		}
	}
	return err
}
...
func (c *AdapterRedis) SetMap(ctx context.Context, data map[interface{}]interface{}, duration time.Duration) error {
	...
	if duration == 0 {
		err := c.redis.MSet(ctx, conv.Map(data))
		if err != nil {
			return err
		}
	}
...
func (c *AdapterRedis) Get(ctx context.Context, key interface{}) (*vars.Var, error) {
	return c.redis.Get(ctx, conv.String(key))
}
...
func (c *AdapterRedis) GetOrSet(ctx context.Context, key interface{}, value interface{}, duration time.Duration) (result *vars.Var, err error) {
	result, err = c.Get(ctx, key)
	if err != nil {
		return nil, err
	}
	if result.IsNil() {
		return vars.New(value), c.Set(ctx, key, value, duration)
	}
	return
}
...
// It deletes the `key` if `duration` < 0 or given `value` is nil, but it does nothing
// if `value` is a function and the function result is nil.
func (c *AdapterRedis) GetOrSetFunc(ctx context.Context, key interface{}, f Func, duration time.Duration) (result *vars.Var, err error) {
	v, err := c.Get(ctx, key)
	if err != nil {
		return nil, err
	}
...
// writing mutex lock for concurrent safety purpose.
func (c *AdapterRedis) GetOrSetFuncLock(ctx context.Context, key interface{}, f Func, duration time.Duration) (result *vars.Var, err error) {
...
func (c *AdapterRedis) Contains(ctx context.Context, key interface{}) (bool, error) {
	n, err := c.redis.Exists(ctx, conv.String(key))
	if err != nil {
		return false, err
	}
	return n > 0, nil
...
func (c *AdapterRedis) Keys(ctx context.Context) ([]interface{}, error) {
	...
	return conv.Interfaces(keys), nil
}
...
func (c *AdapterRedis) Update(ctx context.Context, key interface{}, value interface{}) (oldValue *vars.Var, exist bool, err error) {
	var (
		v        *vars.Var
		oldPTTL  int64
		redisKey = conv.String(key)
	...
	if oldPTTL == -1 {
		...
		_, err = c.redis.Set(ctx, redisKey, value, redis.SetOption{TTLOption: redis.TTLOption{PX: conv.PtrInt64(oldPTTL)}})
	}
...
func (c *AdapterRedis) GetExpire(ctx context.Context, key interface{}) (time.Duration, error) {
	pttl, err := c.redis.PTTL(ctx, conv.String(key))
	if err != nil {
		return 0, err
	}
	switch pttl {
...
func (c *AdapterRedis) Remove(ctx context.Context, keys ...interface{}) (lastValue *vars.Var, err error) {
	if len(keys) == 0 {
		return nil, nil
	}
	// Retrieves the last key value.
	if lastValue, err = c.redis.Get(ctx, conv.String(keys[len(keys)-1])); err != nil {
		return nil, err
	}
	// Deletes all given keys.
	_, err = c.redis.Del(ctx, conv.Strings(keys)...)
	return
}
